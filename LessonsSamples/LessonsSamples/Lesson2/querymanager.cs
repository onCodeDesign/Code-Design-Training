namespace FreshServices
{
   class QueryManager
   {
      ...
      const string m_addCustomer = "addCustomer";
      const string m_authenticate = "authenticate";
      const string m_checkLatestMap = "checkLatestMap";
      const string m_checkMapVersion = "checkMapVersion";
      const string m_getClientInfo = "getClientInfo";
      const string m_getContentList = "getContentList";
      const string m_getContentURL = "getContentURL";
      const string m_getFtpUploadURL = "getFtpUploadURL";
      const string m_getRegistrationList = "getRegistrationList";
      const string m_getSoftwareVersion = "getSoftwareVersion";
      const string m_getSoftwareVersion2 = "getSoftwareVersion2";
      const string m_getVoucherInfo = "getVoucherInfo";
      const string m_redeemVoucher = "redeemVoucher";
      const string m_registerAdditionalContent = "registerAdditionalContent";
      const string m_registerDevice = "registerDevice";
      const string m_updateLatestMap = "updateLatestMap";
      const string m_updateRegistration = "updateRegistration";
      const string m_upgradeProduct = "upgradeProduct";
      const string m_resolveSite = "resolveSite";
      ...
      public bool Init(ISettingsManagerPtr settingsPtr, IDeviceManagerPtr devicePtr, IAccountManagerPtr accountPtr)
      {
      ...
      public bool SendQuery(FreshServerCommands commandName)
      {
         QMap<string, string> tmpMap;
         return SendQuery(commandName, tmpMap);
      }

      public bool SendQuery(FreshServerCommands commandName, QMap<string, string> querryMap)
      {
         ...
         string clientTypeNavSoft = tmpGetInfoResp.software.brand + "_" + tmpGetInfoResp.software.platform;
         string clientVersionNavSoft = tmpGetInfoResp.software.version;
         string deviceUID_ = tmpGetInfoResp.software.deviceUID;
         string hardwareID_ = tmpGetInfoResp.software.hardwareID;
         // TODO: this was only comment for some tests, uncomment when ready
         // string querryString = serverSettings.m_FreshServerAddress + "?cmd=";
         string querryString = "http://st-mysite-upd-5.mobility-platform.de/upd/api/client?cmd=";
         // TODO: get this from the proper place
         string querryStringShop = "http://www.mysite.com/ws/resolveSite";
         switch (commandName)
         {
            case ADD_CUSTOMER:
         ...
         if (querryMap.size() > 0)
         {
            foreach (QMap<string,string> mapIt in querryMap)
            {
      ...
      public void GetReply_ADD_CUSTOMER(AddCustomerResp_T& servResp)
      {
         servResp = m_addCustomerResp;
      }

      public void GetReply_AUTHENTICATE(AuthenticateResp_T& servResp)
      {
         servResp = m_authenticateResp;
      }

      public void GetReply_CHECK_LATEST_MAP(CheckLatestMapResponse_T& servResp)
      {
         servResp = m_checkLatestMapResp;
      }

      public void GetReply_CHECK_MAP_VERSION(CheckMapVersionResp_T& servResp)
      {
         servResp = m_checkMapVersionResp;
      }

      public GetReply_GET_CLIENT_INFO(GetClientInfoResponse_T& servResp)
      {
         servResp = m_getClientInfoResp;
      }

      public void GetReply_GET_CONTENT_LIST(ContenList_T& servResp)
      {
         servResp = m_getContentListResp;
      }

      public void GetReply_GET_CONTENT_URL(GetContentUrlResponse_T& servResp)
      {
         servResp = m_getContentUrlResp;
      }

      public void GetReply_GET_FTP_UPLOAD_URL(GetFtpUploadUrlResp_T& servResp)
      {
         servResp = m_getFtpUploadUrlResp;
      }

      public void GetReply_GET_REGISTRATION_LIST(GetRegistrationList_T& servResp)
      {
         servResp = m_getRegistrationListResp;
      }

      public void GetReply_GET_SOFTWARE_VERSION(GetSoftwareVersion_T& servResp)
      {
         servResp = m_getSoftwareVersionResp;
      }

      public void GetReply_GET_SOFTWARE_VERSION2(GetSoftwareVersion2_T& servResp)
      {
         servResp = m_getSoftwareVersion2Resp;
      }

      public void GetReply_GET_VOUCHER_INFO(GetVoucherInfoResp_T& servResp)
      {
         servResp = m_getVoucherInfoResp;
      }

      public void GetReply_REDEEM_VOUCHER(RedeemVoucherResp_T& servResp)
      {
         servResp = m_redeemVoucherResp;
      }

      public void GetReply_REGISTER_ADDITIONAL_CONTENT(RegisterAdditionalContentResp_T& servResp)
      {
         servResp = m_registerAdditionalContentResp;
      }

      public void GetReply_REGISTER_DEVICE(RegisterDeviceResponse_T& servResp)
      {
         servResp = m_registerDeviceResp;
      }

      public void GetReply_UPDATE_LATEST_MAP(AddCustomerResp_T& servResp)
      {
         servResp = m_updateLatestMapResp;
      }

      public void GetReply_UPDATE_REGISTRATION(RegisterDeviceResponse_T& servResp)
      {
         servResp = m_updateRegistrationResp;
      }

      public void GetReply_UPGRADE_PRODUCT(RegisterDeviceResponse_T& servResp)
      {
         servResp = m_upgradeProductResp;
      }

      public void GetReply_RESOLVE_SITE(ResolveSiteResp_T& servResp)
      {
